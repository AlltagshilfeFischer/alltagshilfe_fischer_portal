          ...
          <div className="flex">
            ...
            {weekdays.map((date) => {
              ...
              return (
                <div
                  ...
                  className={cn("flex-1 min-w-[180px] p-2 text-center border-r", isToday && "bg-primary/5")}
                >
                  <div className={cn("text-sm font-medium", isToday && "text-primary")}>
                    {format(date, "EEEE", { locale: de })}
                  </div>
                  <div className={cn("text-2xl font-bold", isToday && "text-primary")}>{format(date, "d")}</div>
                  <div className="text-xs text-muted-foreground">{format(date, "MMM yyyy", { locale: de })}</div>
                </div>
              );
            })}
            ...
            {weekendDays.map((date) => {
              ...
              return (
                <div
                  key={date.toISOString()}
                  className={cn("w-24 flex-shrink-0 p-2 text-center border-r bg-muted/30", isToday && "bg-primary/10")}
                >
                  <div className="text-xs font-medium text-muted-foreground">{format(date, "EEE", { locale: de })}</div>
        ...
        <div>
          {employees.map((employee) => (
            <div key={employee.id} className="flex border-b hover:bg-muted/20 transition-colors">
              ...
              <div
                className="flex-shrink-0 p-3 border-r bg-card sticky left-0 z-10"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">Max: {employee.max_termine_pro_tag} Termine/Tag</div>
                  </div>
                </div>
              </div>
              ...
              {weekdays.map((date) => {
                ...
                return (
                  <div key={date.toISOString()} className="flex-1 min-w-[180px] border-r">
                    <EnhancedDropZone
                      id={dropZoneId}
                      ...
                      className={cn("min-h-[120px] p-2 space-y-2", isSameDay(date, new Date()) && "bg-primary/5")}
                    >
              ...
              {weekendDays.map((date) => {
                ...
                return (
                  <div key={date.toISOString()} className="w-24 flex-shrink-0 border-r bg-muted/30">
                    <EnhancedDropZone
                      id={dropZoneId}
                      isEmpty={dayAppointments.length === 0}
                      isWeekend={true}
